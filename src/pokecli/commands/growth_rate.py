...
app = typer.Typer(help="Search and browse Growth Rates (experience curves).")
console = Console()
...
def get(
    ctx: typer.Context,
    name_or_id: str = typer.Argument(..., help="Growth rate name or ID"),
    no_cache: bool = typer.Option(False, "--no-cache", help="Skip local cache"),
    format: str = typer.Option(
        "table", "--format", help="Output format: table or json"
    ),
) -> None:
    """Get details about a Growth Rate."""
...
def list_growth_rates(
    ...
    """List Growth Rates with pagination."""
    client = ctx.obj["client"]
