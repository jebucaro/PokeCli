...
from pokecli.commands._helptext import FORMAT, LIMIT, NATURE_NAME_OR_ID, NO_CACHE, OFFSET
...
app = typer.Typer(help="Look up Pokemon natures and their stat effects.")
console = Console()
...
def get(
    ctx: typer.Context,
    name_or_id: str = typer.Argument(..., help=NATURE_NAME_OR_ID),
    no_cache: bool = typer.Option(False, "--no-cache", help=NO_CACHE),
    format: str = typer.Option("table", "--format", help=FORMAT),
) -> None:
    """Show a nature's stat changes and flavor preferences."""
...
def list_natures(
    ...
    """Browse natures with pagination."""
    client = ctx.obj["client"]
    result = fetch_list(client, "nature", limit, offset, err_console)
