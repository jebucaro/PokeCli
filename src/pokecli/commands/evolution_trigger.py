...
app = typer.Typer(help="Search and browse Evolution Triggers.")
console = Console()
...
def get(
    ctx: typer.Context,
    name_or_id: str = typer.Argument(..., help="Evolution trigger name or ID"),
    no_cache: bool = typer.Option(False, "--no-cache", help="Skip local cache"),
    format: str = typer.Option(
        "table", "--format", help="Output format: table or json"
    ),
) -> None:
    """Get details about an Evolution Trigger."""
...
def list_evolution_triggers(
    ...
    """List Evolution Triggers with pagination."""
    client = ctx.obj["client"]
    render_list(
