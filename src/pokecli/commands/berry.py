...
from pokecli.commands._helptext import BERRY_NAME_OR_ID, FORMAT, LIMIT, NO_CACHE, OFFSET
...
app = typer.Typer(help="Look up berries, flavors, and natural gift details.")
console = Console()
...
def get(
    ...
    if format == "json":
        ...
        from pokecli.display.toon_schemas import berry_toon
        from pokecli.display.hints import get_hints, format_hints_toon
        hints = get_hints("berry.get", {"name": berry.name})
        fields = berry_toon(berry)
        print_toon(toon_single("berry", fields))
...
def list_berries(
    ...
    """Browse berries with pagination."""
    client = ctx.obj["client"]
    result = fetch_list(client, "berry", limit, offset, err_console)
    ...
    hints = get_hints("berry.list", {"resource": "berry", "first_name": first_name})
    if format == "toon":
        ...
        print_toon(toon_list("berries", schema_fields, rows, total=result.count))
        hint_text = format_hints_toon(hints)
        if hint_text:
            print_toon("\n" + hint_text)
    else:
